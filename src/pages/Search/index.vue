    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-show="searchParams.categoryName">{{searchParams.categoryName}}<i @click="removeCateName">x</i></li>
            <!-- 关键字的面包屑 -->
            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                ...
                <li :class="{active:isTwo}" @click="changeOrder(2)">
                  <a>销量<span v-show="isTwo" class="iconfont" :class="{'icon-down':isDesc,'icon-up':isAsc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
import { mapGetters, mapState } from 'vuex';
  ...
  export default {
    ...
    methods: {
      ...
      removeAttr(index){
        this.searchParams.props.splice(index,1);//第index个 删1个
        this.getData();
      },
      changeOrder(index){
        ...
        let order = '';
        if(index == originFlag)
          order = `${index}:${originSort=='desc'?'asc':'desc'}`
        else{
          order = `${index}:${originSort=='desc'?'asc':'desc'}`
        }
      ...
      goDetail(id){
        this.$router.push({name:'detail',path:'/detail',params:{skuId:id}});
      }
    },
    computed:{
